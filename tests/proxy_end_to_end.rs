...
use support::{
    capture_channel, capture_request, load_test_config, load_test_config_with_billing_timeout,
    signed_token, spawn_chunked_upstream, spawn_upstream,
};
...
#[tokio::test]
async fn proxy_route_forwards_request_uses_claim_api_and_injects_request_id()
-> Result<(), Box<dyn std::error::Error>> {
...
async fn proxy_route_preserves_raw_encoded_path_and_query() -> Result<(), Box<dyn std::error::Error>>
{
    ...
    let response = app
        .oneshot(
            Request::builder()
                .uri("/proxy/v1/files/a%2Fb?expand=%2F&literal=%252F")
                .header("authorization", format!("Bearer {token}"))
                .body(Body::empty())?,
        )
        .await?;
...
#[tokio::test]
async fn proxy_route_preserves_trailing_proxy_slash() -> Result<(), Box<dyn std::error::Error>> {
    let (sender, rx) = capture_channel();
    let upstream = Router::new()
        .route("/api/", any(capture_request))
        .route("/api/{*path}", any(capture_request))
