...
#[test]
fn add_api_with_username_only_basic_auth_omits_password_key()
-> Result<(), Box<dyn std::error::Error>> {
    let temp_dir = tempdir()?;
    let config_path = temp_dir.path().join("gate-agent.toml");
    fs::write(
...
"#,
    ...
    let basic_auth = parsed
        .get("apis")
        .and_then(|value| value.get("billing"))
        .and_then(|value| value.get("basic_auth"))
        .and_then(toml::Value::as_table)
        .expect("basic_auth table");

    assert_eq!(
        basic_auth.get("username").and_then(toml::Value::as_str),
        Some("billing-user")
    );
...
#[test]
fn api_upsert_debug_redacts_basic_auth_password() {
    let rendered = format!(
        "{:?}",
        ApiUpsert {
            name: "billing".to_string(),
    ...
    assert!(rendered.contains("billing-user"), "{rendered}");
    assert!(
        rendered.contains("[REDACTED]") || !rendered.contains("password"),
        "{rendered}"
    );
    assert!(!rendered.contains("billing-pass"), "{rendered}");
}
